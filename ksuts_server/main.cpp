...
bool readDevices(const CFG& cfg)
{
  ...
  try {
    ...
    if(cfg.single_device_mode)
      serr = PG->execSQL(QString(SQL_SELECT_SINGLE_CONFIGURED_DEVICE).arg(cfg.single_device_index), q);
    else
      serr = PG->execSQL(SQL_SELECT_CONFIGURED_DEVICES, q);
...
//  config.id = q->value("device_id").toInt();
  ...
  config.single_device_mode = cfg.single_device_mode;
