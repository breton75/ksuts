...
struct Type_0x13 {
  ...
  {
    byte0.direction = direction; // это значение не изменяется для каждого направления
    
    set(state, regim, urs_state, otkl_vent);
  }
  
  void set(quint8 state, quint8 regim, quint8 urs_state, quint8 otkl_vent)
  {
    byte1.state = state;
    byte1.regim = regim;
    byte2.urs_state = urs_state;
    byte2.otkl_vent = otkl_vent;    
  } 
