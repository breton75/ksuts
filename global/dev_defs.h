...
public:
  ...
  bool parse()
  {
    ...
    try {
      ...
      if(!parser.parse(params_list)) _exception.raise(QString("Wrong params: %1").arg(_params_string));      
  
      _serial_params.baudrate = parser.value(P_SERIAL_BAUDRATE).toUInt(&ok); 
      if(!ok) _exception.raise(QString("Wrong params: %1").arg(P_SERIAL_BAUDRATE));
      
      d = parser.value(P_SERIAL_DATABITS).toUInt(&ok);
      if(!(ok & DataBits.contains(QSerialPort::DataBits(d)))) _exception.raise(QString("Wrong params: %1").arg(P_SERIAL_DATABITS));
      _serial_params.databits = QSerialPort::DataBits(d);
          
      d = parser.value(P_SERIAL_FLOWCTRL).toUInt(&ok);
      if(!(ok & FlowControls.contains(QSerialPort::FlowControl(d)))) _exception.raise(QString("Wrong params: %1").arg(P_SERIAL_FLOWCTRL));
      ...
      if(!ok) _exception.raise(QString("Wrong params: %1").arg(P_SERIAL_PARITY));
      _serial_params.parity = QSerialPort::Parity(d);
          
      d = parser.value(P_SERIAL_STOPBITS).toUInt(&ok);
      if(!ok) _exception.raise(QString("Wrong params: %1").arg(P_SERIAL_STOPBITS)); 
