#include "editwindow.h"
#include "ui_editwindow.h"

EditWindow::EditWindow(int mode, QWidget *parent) :
  QMainWindow(parent),
  ui(new Ui::EditWindow)
{
  ui->setupUi(this);

  _mode = mode;

  setWindowTitle(QString("Автозапуск КСУТС %1").arg(APP_VERSION));
  setWindowIcon(QIcon(QPixmap(":/png/png/view-form.png")));

  // чтобы настройки писались и читались в cfg файл
  AppParams::setCurrentFormat(QSettings::NativeFormat);

  /// список команд для выполнения ВНАЧАЛЕ
  int i = 0;
  while(1) {

      QString cmd = AppParams::readParam(this, "General", QString(COMMAND_BEFORE_n).arg(i++), "").toString();
      if(cmd.isEmpty())
          break;

      ui->textExecuteBefore->insertPlainText(cmd + '\n');

  }

  /// ждать ли postgres и aggreagte
  ui->checkWaitPostgres->setChecked(AppParams::readParam(this, "General", PARAM_WAIT_POSTGRES, "true").toBool());
  ui->checkWaitAggregate->setChecked(AppParams::readParam(this, "General", PARAM_WAIT_AGGREGATE, "true").toBool());

  /// список команд для выполнения ПОСЛЕ
  i = 0;
  while(1) {

      QString cmd = AppParams::readParam(this, "General", QString(COMMAND_AFTER_n).arg(i++), "").toString();
      if(cmd.isEmpty())
          break;

      ui->textExecuteAfter->insertPlainText(cmd + '\n');

  }

  /// список серверов aggregate
  i = 0;
  while(1) {

      QString cmd = AppParams::readParam(this, "General", QString(PARAM_WIDGET_HOST_n).arg(i++), "").toString();
      if(cmd.isEmpty())
          break;

      ui->textAggOrder->insertPlainText(cmd + '\n');

  }

  /// параметры для запуска виджета
  _widget_user = AppParams::readParam(this, "General", QString(PARAM_WIDGET_USER), "admin").toString();
  _widget_pass = AppParams::readParam(this, "General", QString(PARAM_WIDGET_PASS), "admin").toString();
  _widget_name = AppParams::readParam(this, "General", QString(PARAM_WIDGET_NAME), "Start_GEU").toString();
  _widget_xms = AppParams::readParam(this, "General", QString(PARAM_WIDGET_XMS), QString("1500")).toString();
  _widget_xmx = AppParams::readParam(this, "General", QString(PARAM_WIDGET_XMX), QString("4500")).toString();

  ui->textAggParams->insertPlainText(QString("user: %1\n").arg(_widget_user));
  ui->textAggParams->insertPlainText(QString("pass: %1\n").arg(_widget_pass));
  ui->textAggParams->insertPlainText(QString("widget: %1\n").arg(_widget_name));
  ui->textAggParams->insertPlainText(QString("xms: %1\n").arg(_widget_xms));
  ui->textAggParams->insertPlainText(QString("xmx: %1\n").arg(_widget_xmx));

}

EditWindow::~EditWindow()
{
    AppParams::saveParam(this, "General", PARAM_WAIT_POSTGRES,      ui->checkWaitPostgres->isChecked());
    AppParams::saveParam(this, "General", PARAM_WAIT_AGGREGATE,     ui->checkWaitAggregate->isChecked());

    QStringList cmdBeforeList = ui->textExecuteBefore->toPlainText().split('\n');
    for (int i = 0; i < cmdBeforeList.count(); i++)
      AppParams::saveParam(this, "General", QString(COMMAND_BEFORE_n).arg(i),  cmdBeforeList.at(i));

    QStringList cmdAfterList = ui->textExecuteAfter->toPlainText().split('\n');
    for (int i = 0; i < cmdAfterList.count(); i++)
      AppParams::saveParam(this, "General", QString(COMMAND_AFTER_n).arg(i),  cmdAfterList.at(i));

    QStringList aggList = ui->textAggOrder->toPlainText().split('\n');
    for (int i = 0; i < aggList.count(); i++)
      AppParams::saveParam(this, "General", QString(PARAM_WIDGET_HOST_n).arg(i),  aggList.at(i));



  delete ui;
}
