...
bool SvDeviceList::readDevices(TreeModel* model)
{
  ...
  try {
    ...
    while(q->next()) {
      ...
      root->child(child_count)->index = q->value("device_index").toInt();
...
void SvDeviceList::on_bnAddSelected_clicked()
{
  ...
  for(QModelIndex idx: ui->tableAvailable->selectionModel()->selectedRows(0))
      devices_list += QString("%1,").arg(_model_available->itemFromIndex(idx)->index);
...
void SvDeviceList::on_bnRemoveSelected_clicked()
{
  ...
  for(QModelIndex idx: ui->tableCurrent->selectionModel()->selectedRows(0))
      devices_list += QString("%1,").arg(_model_current->itemFromIndex(idx)->index);
...
void SvDeviceList::editDevice()
{
  ...
  DEVICE_UI = new SvDeviceEditor(this, _model_current->itemFromIndex(ui->tableCurrent->currentIndex())->index);
